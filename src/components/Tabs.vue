<template>
  ...
  <div class="px-4 sm:px-0">
    ...
    <div class="hidden sm:block">
      <nav class="relative z-0 rounded shadow-lg flex divide-x divide-zinc-200" aria-label="Tabs">
        <a v-for="(tab, tabIdx) in tabs" :key="tab.name" :href="tab.href"
           :aria-current="tab.current ? 'page' : undefined"
           :class="[tab.current ? 'text-zinc-400' : 'text-zinc-100 brand-transition hover:text-zinc-900 hover:bg-zinc-300', tabIdx === 0 ? 'rounded-l' : '', tabIdx === tabs.length - 1 ? 'rounded-r' : '', 'group relative min-w-0 flex-1 overflow-hidden bg-zinc-900 py-4 px-6 text-sm font-medium text-center focus:z-10']">
          <span>{{ tab.name }}</span>
            ...
            <div class="flex space-x-3">
              ...
              <div class="flex-shrink-0 self-center flex">
                <Menu as="div" class="relative inline-block text-left">
                  <div>
                    <MenuButton
                        class="-m-2 p-2 rounded-full flex items-center text-zinc-400 hover:text-zinc-200">
                  ...
                  <transition enter-active-class="transition ease-out duration-100"
                    ...
                    <MenuItems
                        class="origin-top-right absolute right-0 mt-2 w-56 roundedadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                      <div class="py-1">
            ...
            <div class="flex space-x-6">
              <span class="inline-flex items-center text-sm">
                <button type="button" class="inline-flex space-x-2 text-zinc-400 hover:text-zinc-200">
                  ...
                  <span class="font-medium text-zinc-900">{{ content.likes }}</span>
              ...
              <span class="inline-flex items-center text-sm">
                <button type="button" class="inline-flex space-x-2 text-zinc-400 hover:text-zinc-200">
                  ...
                  <span class="font-medium text-zinc-900">{{ content.replies }}</span>
              ...
              <span class="inline-flex items-center text-sm">
                <button type="button" class="inline-flex space-x-2 text-zinc-400 hover:text-zinc-200">
                  ...
                  <span class="font-medium text-zinc-900">{{ content.views }}</span>
            ...
            <div class="flex text-sm">
              <span class="inline-flex items-center text-sm">
                <button type="button" class="inline-flex space-x-2 text-zinc-400 hover:text-zinc-200">
                  ...
                  <span class="font-medium text-zinc-900">Share</span>
                </button>
              </span>
            </div>
          </div>
